...
## API Key 없이 빠르게 확인하기 (수동/브라우저)

API 키를 설정하지 않아도, 오피넷 웹사이트에서 *수동으로* 지역별 가격을 확인할 수 있습니다.

- 오피넷 지역별 가격 조회: https://www.opinet.co.kr/searRgSelect.do

> 참고: 이 프로젝트(및 CLI/Skill)는 기본적으로 오피넷 *Open API*를 호출하는 방식이라, 자동/반복 조회를 하려면 OPINET API Key가 필요합니다. (아래 설정 섹션 참고)

### (옵션, 실험적) 브라우저 기반 탐색/자동화

- *OpenClaw 사용자라면*: OpenClaw의 내장 브라우저 기능으로 위 페이지를 직접 탐색해 확인하는 방식도 가능합니다.
- *Playwright/Selenium 등* 브라우저 자동화 도구로도 시도할 수는 있으나,
  - 웹 UI 변경/캡차/접근 제한 등으로 동작이 불안정할 수 있어 *best effort(성공 보장 없음)*이며,
  - 본 프로젝트의 기본 지원 범위는 **오피넷 Open API 기반(=API Key 필요)** 입니다.
